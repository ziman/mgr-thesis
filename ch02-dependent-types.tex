...
assistant and greatly helps writing both proofs and Agda code in general. A~(very) brief guide
can be found on the Agda wiki \cite{agda-mode}.
...
Therefore conversely, if we have a \ident{refl : a $\equiv$ b}, it must be the case that \ident{a}
...
There is one syntactic feature of Agda left that we have not used yet but that will
...
the first element in the list satisfying the given predicate. One way to do it is
to use the library function \ident{filter} that, given a predicate and
a list, returns the list of \emph{all} elements satisfying the given predicate.

One way is to define an auxiliary function and compose it with the function
\ident{filter}:
\begin{code}
...
\begin{code}
  ...
  \... | y :: _ = just y
\end{code}
The ellipses are replaced by the patterns that precede the keyword \ident{with} in the first
