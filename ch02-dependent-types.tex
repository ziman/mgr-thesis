...
are usually possible even in non-functional mainstream languages, often called \emph{enums}.

In Agda however, data constructors may take arguments, as shown in the following snippet.
\begin{code}
  data Observation : Set where
    LightsOn : Bool -> Measurement
    PersonsPresent : \bN -> Measurement
\end{code}
...
the natural numbers, let us declare the type of lists (of natural numbers).
\begin{code}
...
declaration as parameters that may be chosen arbitrarily. As already mentioned, in the case
...
a~\emph{type family} by adding another argument to the type constructor \ident{List}.
The newly added argument will denote the length of the list.
\begin{code}
  data Vec (A : Set) : Nat -> Set where
    \NIL : Vec A zero
    _::_ : forall {n} -> A -> Vec A n -> Vec A (1 + n)
\end{code}
The above code says that the constructor \ident{[\,]} constructs a list of any given type
with zero length; the constructor \ident{\_::\_}, for any (implicit) length \ident{n},
takes an element of the type \ident{A} and an appropriately typed list of length \ident{n},
yielding a list whose length is \ident{1 + n}.

Note several important differences of the newly added \emph{index} to the \emph{parameter}
\ident{A}:
\begin{itemize}

	\item Parameters appear to the left of the semicolon in the declaration head
	...
	\item In the above example, all data constructors must result in the type \ident{Vec A n}
		for some value of \ident{n}. The way the declaration is written, there is no way to have
		a~constructor construct a~value of the type \ident{Vec (Maybe A) n}.
		
	\item Recursive occurrences of a data type must use the same parameters.%
		\footnote{In Agda, this is a bit complicated. Indices are \emph{usually} written
			to the right of the semicolon in the declaration head. However, Agda allows
			using different parameters in recursive occurrences of a data type in its
			constructors.}
		
	\item given by user vs. given by constructors
