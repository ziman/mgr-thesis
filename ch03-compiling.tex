...
values, now we are going to push exception handlers on the stack, too. We will
do this as follows.

We begin by extending the type of stack shapes. A stack shape is no longer a plain
list of types (that is, elements of the universe \ident{U}); instead, we distinguish
between \emph{values} and \emph{handlers} pushed on the stack.
\begin{code}
  data Item : Set where
    Val : U -> Item
    Han : U -> Item
...
\noindent A value, denoted by \ident{Val u}, is an actual value of the type
...
This corresponds to the new type of stacks,
\begin{code}
  data Stack : Shape -> Set where
    snil : Stack []
...
Note that we are pushing whole strands of code on the stack; single stack items can
be arbitrarily large, which contradicts one of our design principles -- that the
code must be executable on a dumb stack machine (Introduction, page~\pageref{objectives}).

Hutton and Wright introduce three new instructions of the virtual machine:
