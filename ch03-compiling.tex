...
%\inset{First, Hutton and Wright propose altering the type of stacks because besides
%values, now we are going to push exception handlers on the stack, too.}
\begin{code}
  data Item : Set where
    Val : U -> Item
...
\begin{itemize}
	...
	\item exception handling, where we keep additional state variables besides the stack.
\end{itemize}
While in real machines, the above distinction would result in yet another state variable
...
\inset{We need to know whether there is a handler on the stack and what type it has.}
\begin{code}
...
\inset{We also need to know what shape the unwound stack will have.}
\begin{code}
...
\noindent Then we can define the data types: the resumption point and, finally,
the type of machine states.

\inset{This structure holds information needed to resume computation when the machine
...
\inset{This finally leads to the data type of machine states, which are twofold.}
\begin{code}
