...
To be put it precisely and explicitly, the state of the virtual machine includes
these components:
\begin{itemize}
...
One approach to execution not requiring any additional flags and variables would
be extending the stack type with a new constructor;
...
This approach is actually an adaptation of the one presented by Hutton and Wright,
except that we make the state completely explicit in order to adhere to our objectives.
We discard our attempts with placeholders and return to the original stack representation:
\begin{code}
  data Stack : Shape -> Set where
    snil : Stack []
    _\scons\_ : forall {u s} -> el u -> Stack s -> Stack (Val u :: s)
...
\noindent However, we extend the \emph{state} of the machine instead. We will distinguish
...
\begin{itemize}
	...
	\item exception handling mode, where we keep additional state variables besides the stack.
		This mode is also called \emph{stack unwinding} because
		in this mode, the machine just keeps discarding values from the top of the stack
		until it finds a suitable exception handler.
\end{itemize}
...
First, we need to know whether there is an appropriate handler on the stack and what type it has.
\begin{code}
  -- Get the type of the &\cident{n}&\-th top\-most handler in the Shape.
  -- Return &\cident{nothing}& if there is no handler.
...
\begin{code}
  ...
  -- Return the empty shape if there is no handler.
...
\noindent Then we can define the data types; first the resumption point, which
represents information needed to resume computation after skipping the instructions
belonging to the exception-throwing expression.
\begin{code}
...
The machine has two modes of execution, as hinted in \cite[p.~7]{gmh:exceptions}.

The normal mode is exactly what one would expect: executing instructions on a stack,
nothing surprising.

What we need to discuss is the exception-handling mode. When an exception is thrown,
...
Note that handler frames may be nested since catch-expressions may also
be nested arbitrarily.

Now that handler frames are defined, we can describe exception handling quite
concisely: discard the innermost handler frame, remembering the associated
handler, and then execute the handler as ordinary code on the resulting stack.
