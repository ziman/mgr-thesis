...
\begin{itemize}
	\item The parameter~\ident{n} describes how many handler frames we need to
		...
		\todo{This is exactly the point where Hutton \& Wright use implied stacks.}
...
\todo{Dependent types advocacy: only one case for UNMARK.}

\todo{Trivial exc. processing / instruction skipping only once for each instruction.}

\todo{No implicit stacks, only structural tail recursion.}

\todo{Both modes of execution are essentially the same; both contain stack, the error mode
contains a number and a handler. Thus only the handler violates the simplicity requirement.}
