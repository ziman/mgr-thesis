...
Note that any \emph{guarded expression} is
...
cases with the wildcard pattern \ident{\_}. However, if we do that, Agda
complains because it cannot check that the types involved are correct
...
resume execution at. This maps better to real-world machines although we still
...
\todo{Mention jumps and labels done in GMH?}

\todo{Is it better to recurse structurally over something else than code?}

\todo{THROW fakes it, is indexed like a real PUSH instruction and has no effect
on type indices involved -- these behave as if normal execution took place.}

\todo{Correctness not proved: non-termination.}

\subsection{Execution: handlers at \ident{UNMARK}}

\subsubsection{Foo bar baz}
