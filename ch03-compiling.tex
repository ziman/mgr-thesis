...
it means for an implementation to be correct. We then provide a machine-checkable proof that our
implementation is correct and follows this specification.
...
is broad; the same can be achieved by using strict type signatures to rule out the cases
...
We will build upon our introductory example of the simple exceptionless
language introduced in \Fref{sec:simple-language}.  Code in this section will
be modeled after the paper \cite{gmh:exceptions} by Graham Hutton and Joel
Wright.  The authors used Haskell in their development.  This approach was
later formalized by Tobias Nipkow in \cite{nipkow} in Isabelle, an interactive
theorem prover/proof assistant, in exactly the same way; the development even
uses the same lemmas and their numbering as the paper.
...
\todo{We won't do jumps but we will use execution modes that can be obviously
(tm) replaced by jumps.}
...
\begin{itemize}
	\item The parameter~\ident{n} describes how many handler frames we need to
		...
		By the way, this is the point where Hutton \& Wright
		\cite[pg.~7]{gmh:exceptions} use an implicit stack.  In their
		function \ident{skip}, the implicit call stack is used to count
		the nesting levels; and this is perfectly okay when the
		skipping function is mostly symbolic, isolated, and obvious.
...
\todo{Generalized handler frames, maybe?}
...
this shortcoming soon in the Section \ref{sec:handlers-at-unmark}.
...
\todo{Move this to the high-level language definition?}
...
is trivial, we extend our high-level language with the Boolean type, along with
...
\noindent Now we can use these functions as denotation of the new operators.
