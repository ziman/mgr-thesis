...
First, we will need two functions operating on shapes:

\inset{Get the type of the top-most handler in the Shape or return \ident{nothing}
...
\inset{Unwind the shape up to just below the top-most handler or return the empty
shape if there is no handler.}
\begin{code}
...
\noindent Then we can define the resumption point:

\inset{This is a resumption point of normal operation.}
\begin{code}
...
\noindent Finally, we can define the machine state datatype.

\inset{Finally, we can define the machine state datatype.}
\begin{code}
