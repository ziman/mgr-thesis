...
index the type of stack shapes by the list of handlers on the stack, so we get
...
on the stack will change both the shape and its type. However, we will not take
this route as it turns out to be more complicated, while not yielding substantial
...
\begin{itemize}
	...
	\item exception handling mode, where we keep additional state variables besides the stack.
		This mode is also called \emph{stack unwinding} \note{Stack unwinding.} because
...
represents information needed to resume computation after finding the exception handler.
\begin{code}
