...
Hence, this constructor is similar to the combinator \ident{mplus} in
...
universe~\ident{U}); instead, we  will distinguish between \emph{values} and
\emph{handlers} pushed on the stack.  \footnote{Actually, there is even more
	...
	list of handlers on the stack, getting \ident{Shape : List U $\to$
	Set}. In this setup, pushing a value on the stack will change its shape
...
is reflected in extending the \ident{Instr} type, which must now reside in a
...
and Wright. \srcdir{sec3.3-stack-unwinding}
...
this setup without adding the special states or changing how \ident{execInstr} works.
...
why the first argument is a \ident{Shape}, not the number, is that we need to
...
\begin{itemize}
	\item The parameter~\ident{n} describes how many handler frames we need to
		...
		function \ident{skip}, the implicit call stack is used to count
		the nesting levels.
...
The reason that we need to keep track of the nesting depth is that instruction
...
For example, note that since our high-level language is pure,
...
except \ident{MARK} and \ident{UNMARK}.} in the exception-handling mode is
...
\srcdir{sec3.4-handlers-at-unmark}
...
structural. In the code, seen in \Fref{fig:hau-execInstr}, we will use the auxiliary functions
...
\begin{code}
    ...
    execCode (i <| is) st = execCode is (execInstr i st)
\end{code}
\caption{Handlers at \ident{UNMARK}: execution functions}
...
expression \ident{e}, using the following three lemmas.
...
All clauses are trivial: they have a simple \ident{refl} on the right-hand side.
...
the same; however, compared to the compiler in \Fref{sec:hau-compile}, we
...
In the previous section, handlers were attached to \ident{UNMARK}.\\
\quad \ldots
	\= \quad \ident{MARK}
...
The core of execution is given by the function \ident{execInstr} (\Fref{fig:lin-execInstr}),
...
recursive, and, as always, the function \ident{execCode} is a simple left fold over
instructions.
...
This concludes the operational semantics of instructions of the low-level
virtual machine.
...
along with
two new binary operators: the \ident{Leq} operator that compares natural
numbers, and the \ident{And} operator calculating conjunction of two Boolean
values.
...
\noindent In the above type declaration, we just added the \ident{bool}
constructor. Now we need to extend the function \ident{el} with a new clause
covering the case for the newly introduced \ident{bool} constructor.
...
The type of expressions \ident{Exp} remains unchanged; we just need to add the two new
binary operators to the data type \ident{Op} as new, appropriately typed,
constructors.
...
\noindent Recall that a constructor of the type \ident{Op p q r} represents a
binary operator taking two values of the types \ident{p} and \ident{q},
returning a value of the type \ident{r}.

\subsubsection{Denotational semantics}

Since we changed only the operator data type, we need to alter just the
corresponding denotation function \ident{denOp}.
...
\ident{Instr} data type, as shown in \Fref{fig:more-ops-instr}.
...
by adding new clauses to the function \ident{opInstr}. Recall that this
...
as shown in \Fref{fig:additional-execInstr}.
...
\begin{codei}
  ...
  execInstr  THROW     \tick[          	st ] = \x[ zero , unwindStack st zero ]
  execInstr  UNMARK    \tick[ x \scons skp u \scons st ] = \tick[ x \scons st ]
  execInstr  LEQ       \tick[  x \scons y \scons	st ] = \tick[ (x \leq y) \scons st ]
  execInstr  AND       \tick[  x \scons y \scons	st ] = \tick[ (x \and y) \scons st ]
  -- Exception handling: trivial
  execInstr  THROW		\x[     n , st ] = \x[     n , st ]
  execInstr (PUSH x)	\x[     n , st ] = \x[     n , st ]
  execInstr  ADD		\x[     n , st ] = \x[     n , st ]
  ...
  execInstr  HANDLE		\tick[ x \scons han\scons skp\scons st ] = ![ zero , x \scons st ]
\end{codei}
\caption{Additional types and operations: execution functions}
\label{fig:additional-execInstr}
\end{figure}

Note that the only non-trivial addition was the one in the
...
operator lemma called \ident{lemma-op}. In this function, 12 cases per binary
operator are examined so we end up with $2 \times 24$ new trivial\footnote{All
these proof obligations can be discharged using a simple \ident{refl}.}
