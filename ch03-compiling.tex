...
In the case for \ident{Plus}, Agda substitutes the term \ident{Plus} in the
...
of normal forms, which is indicated by \ident{refl}.
...
We will split the lemma by the parameter \ident{c} in two cases: either
\ident{c} is empty or it consists of an instruction and the rest of code. The
...
The second proof uses \ident{compile-distr} recursively to break the execution
of composite code into two stages. This is not circular reasoning because the
first argument of \ident{compile-distr} is structurally smaller in this
application.
...
% TODO: which version?
Beginning with Agda 2.2.4, we can shorten our previous proof considerably
by using the \ident{rewrite} syntax:
...
The \ident{rewrite} construct expands to a specific pattern-matching mechanism
behind the scenes, effectively rewriting subterms of the goal using the provided
equality (which is the recursive application of \ident{compile-distr} here). The
resulting goal is then easily solvable by \ident{refl}.

\subsubsection{Main correctness theorem}

This is the central result that binds together everything we have defined so
far in a single proof of correctness.

This proof formalizes the idea we informally mentioned when we started to write
the compiler: that executing the compiled code for an expression should be
...
\begin{code}
  correctness : forall {u s}
    -> (e : Exp u) (st : Stack s)
    -> exec (compile e) st == denExp e \scons st
\end{code}
