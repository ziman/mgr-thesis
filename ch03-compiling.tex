...
\begin{itemize}
  ...
  \item The other is \ident{Catch}. This constructor takes two expressions of
    the same type, regular value and an exception handler, representing a
    catch-block. If no exception gets thrown in the value, the whole
    catch-block is equivalent to the regular value. Otherwise, it is equivalent
    to the handler value.\footnote{Especially, if both values throw exceptions,
    the catch-block propagates the exception thrown in the handler.}
    \footnote{Hence, this constructor is similar to the combinator
    \ident{mplus} in Haskell, which combines two possibly failing computations
...
need to formalize what the meaning of the two constructors actually is. For
that purpose, we need to alter the function \ident{denExp}

The first change is that we need to express that an exception has been thrown
during evaluation of an expression. The function \ident{denExp} will now return
\ident{Maybe (el u)} instead of \ident{el u} directly, using the value
\ident{nothing} to indicate uncaught exceptions.

The second change is adding pattern cases for the newly added constructors
of \ident{Exp} to the function \ident{denExp}.
