...
occasionally completely different behavior, they can still be described by in a unified way
with a trivial difference if needed, as shown by Thielecke \cite{thielecke01}.
...
are mechanically checked by typecheckers automatically.

What's more, such programs needn't contain only theorems. They usually also exploit
...
hardware, etc., the guarantees and subsequently gains will grow.

In this thesis, we study a specific domain involving specification, compilation and execution
of programs with exceptions, for the reasons stated above.
