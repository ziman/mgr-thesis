...
In 1990, Griffin noticed \cite{griffin90} that the control operator $\mathcal{C}$
used in $\lC$ can be given the type $\neg \neg A \to A$, yielding a \emph{computational
interpretation} of classical logic. However, operationally, this calculus does not match
classical natural deduction \cite{ariola-herbelin}.
...
$\lambda_\mu$ Parigot presented does not readily correspond to classical logic --
